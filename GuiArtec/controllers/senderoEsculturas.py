...
def pruebaMarkers():
    places = []
    rows = db(db.puntosp.id != None).select()
    for row in rows:
        place = {
        'lat': row.lat,
        'lng': row.lng,
        ...
        'stopover' : True,
        'infoWindow': {'content': "<h4 style='color:black;text-align:center'>"+row.nombre+"</h4><p style='color:black;text-align:center'>Direccion: "+row.direccion},
        }
        places.append(place)
...
#Sendero Puntosp
def crearSenderoPuntosP():
    paths = []
    rows = db().select(db.puntosp.ALL,orderby=db.puntosp.id)
    for row in rows:
        path = {
